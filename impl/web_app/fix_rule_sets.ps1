...
try {
    $rules = Invoke-RestMethod -Uri "$baseUrl/api/compliance/rules?active=false" `
        -Method GET `
    ...
    if ($rule.rule_set_ids -and $rule.rule_set_ids.Count -gt 0) {
        ...
        foreach ($ruleSetId in $rule.rule_set_ids) {
            try {
                ...
                Write-Host "  ✗ Failed to add to rule set $($ruleSetId): $_" -ForegroundColor Red
                $errorCount++
            }
        }
...
Write-Host ""
Write-Host "Verify rule_ids are now populated in rule sets." -ForegroundColor Cyan
